...
public:
  ...
  eta_(eta),phi_(phi),testE_(testE) { }
  ...
  MonoCandidate(const MonoCandidate &mc) : 
    ...
    hIso_(mc.hIso_),eta_(mc.eta_),phi_(mc.phi_),testE_(mc.testE_) { } //{ empty >////<} 
...
//	cout<<"ev "<<ev<<", cand number "<<nCandidates<<endl;
	...
	//cut flow events calculating
        ...
        for(int i=0;i<CutFlowCand_TRG.size();i++){
                //cout<<"ev "<<ev<<", cutflow TRG order:"<<CutFlowCand_TRG[i].dEdXSig_<<endl;
        }
...
void MonoAnalyzerReal_2017_1000()
{
	TFile *oFile = new TFile("Monopole_Analyzer_2017_1000.root","recreate");
//        TFile *oFile = new TFile("Monopole_Analyzer.root","recreate");
	...
	tree->SetBranchAddress("egComb_E",&testE);
	...
	vector<MonoCandidate> cand(10);	
		...
		cand[i] = MonoCandidate(
	        ...
	        (*eta)[i],
	        (*phi)[i],
		(*testE)[i]// just test this not match distribution,to see the distribution compare to e55
     		 );
		}
