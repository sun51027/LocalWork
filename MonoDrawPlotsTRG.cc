...
void MonoDrawPlotsTRG()
{
	...
	for(int y=0;y<2;y++){
	  for(int m=0;m<3;m++){
		...
		TDirectory *dir = (TDirectory*)fin->Get("HLT_Photon200");
	...
	FracSatVNstrips.push_back((TH2D*)dir->Get((histogram[i]+"FracSatVNstrips").c_str()));
        ...
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"FracSatVNstrips.pdf").c_str());
	...
	DedXSig.push_back((TH1D*)dir->Get((histogram[i]+"DedXSig").c_str()));
        ...
        leg->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"DedXSig.pdf").c_str());
	...
	F51.push_back((TH1D*)dir->Get((histogram[i]+"F51").c_str()));
        ...
        leg2->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"F51.pdf").c_str());
	...
	HcalIso.push_back((TH1D*)dir->Get((histogram[i]+"HcalIso").c_str()));
        ...
        leg3->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"HcalIso.pdf").c_str());
	...
	//Energy(e55)
        ...
        leg4->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"Energy(e55).pdf").c_str());
	...
	RZcurv.push_back((TH1D*)dir->Get((histogram[i]+"RZcurv").c_str()));
        ...
        leg5->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"RZcurv.pdf").c_str());
	...
	ABCD.push_back((TH2D*)dir->Get((histogram[i]+"ABCD").c_str()));
        ...
        LooseX->Draw("same");
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"ABCD.pdf").c_str());
