...
//////////////////////////////////
//
//    File name: MakeBlindPlot.cc
//	 author: Shih Lin(modified from Mai's MakeBlindPlot.cc)
//	   Date: 2021/1
//	Content: ABCD method
//	Purpose: Background estimate 
//  	  Input: TH2F* MakeBlindPlotSig(TTree *Mono, int Unblind=3)
//
/////////////////////////////////////////////////
...
TH2F* MakeBlindPlotSig(TTree *Mono, int Unblind=3){
    ...
    double xbins[4] = {x[0]/100, x[1]/100, x[2]/100, x[3]/100};//0, 0.58, 0.73,1.05 x-axis
    double ybins[4] = {y[0]/2, y[1]/2, y[2]/2, y[3]/2};//0, 4.5, 8, 30 y-axis
...
// Take from Trim.c
    ...
    cout<<"total Events "<<Mono->GetEntries()<<endl;
    vector<double> key;
    for(int track=0; track< Mono->GetEntries(); track++){
    ...
    for(int i=0; i < key.size();i++)    cout <<"key event "<<key[i]<<endl;
