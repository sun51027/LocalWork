...
#include <string>
using namespace std;
void MonoDraw2016PFMET170_NoiseCleaned()
{
	//create a canvas
...
//		TFile *fin = new TFile (("Test_E55_"+Year[y]+"_"+GeV[m]+".root").c_str());
	//	TFile *fin = new TFile (("Monopole_Analyzer_"+Year[y]+"_"+GeV[m]+".root").c_str());
		...
		TDirectory *PFMET170_NoiseCleaned = (TDirectory*)fin->Get("passHLT_PFMET170_NoiseCleaned");
		PFMET170_NoiseCleaned->cd();
	...
	*/
	...
	FracSatVNstrips.push_back((TH2D*)PFMET170_NoiseCleaned->Get((histogram[i]+"FracSatVNstrips").c_str()));
        ...
        canvas->SaveAs(("./Plots/"+Year[y]+"MC"+Mass[m]+"/PFMET170_NoiseCleaned/"+plotName[i]+"FracSatVNstrips.pdf").c_str());
	
	//create dedx
	DedXSig.push_back((TH1D*)PFMET170_NoiseCleaned->Get((histogram[i]+"DedXSig").c_str()));
        ...
        leg->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+"MC"+Mass[m]+"/PFMET170_NoiseCleaned/"+plotName[i]+"DedXSig.pdf").c_str());
	
	//create f51
	F51.push_back((TH1D*)PFMET170_NoiseCleaned->Get((histogram[i]+"F51").c_str()));
        ...
        leg2->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+"MC"+Mass[m]+"/PFMET170_NoiseCleaned/"+plotName[i]+"F51.pdf").c_str());
	

	string test = Year[y]+" Mass "+Mass[m]+" 99814 Entries";
	//create hiso
	HcalIso.push_back((TH1D*)PFMET170_NoiseCleaned->Get((histogram[i]+"HcalIso").c_str()));
        ...
        leg3->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+"MC"+Mass[m]+"/PFMET170_NoiseCleaned/"+plotName[i]+"HcalIso.pdf").c_str());

	//Energy(e55)
        Energy.push_back((TH1D*)PFMET170_NoiseCleaned->Get((histogram[i]+"E55").c_str()));
        ...
        leg4->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+"MC"+Mass[m]+"/PFMET170_NoiseCleaned/"+plotName[i]+"Energy(e55).pdf").c_str());


	//Rz curv
	RZcurv.push_back((TH1D*)PFMET170_NoiseCleaned->Get((histogram[i]+"RZcurv").c_str()));
        ...
        leg5->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+"MC"+Mass[m]+"/PFMET170_NoiseCleaned/"+plotName[i]+"RZcurv.pdf").c_str());
	

	double dEdXSigCut_ = 9;
	double f51Cut_ = 0.85;
	ABCD.push_back((TH2D*)PFMET170_NoiseCleaned->Get((histogram[i]+"ABCD").c_str()));
        ...
        LooseX->Draw("same");
        canvas->SaveAs(("./Plots/"+Year[y]+"MC"+Mass[m]+"/PFMET170_NoiseCleaned/"+plotName[i]+"ABCD.pdf").c_str());
